    ...
    <footer className="site-footer">
      <div className="page-width">
        <div className="footer-main">
          ...
          <div className="footer-links"><div><h3>Programs</h3><a href="#postpartum">Postpartum Recovery</a><a href="#programs">Core Foundations</a><a href="#for-men">For Men</a><a href="#programs">Private Coaching</a></div><div><h3>ShapeHaus</h3>{NAV_LINKS.slice(0, 4).map((link) => <a href={link.href} key={link.href}>{link.label}</a>)}</div><div><h3>Support</h3><a href={`mailto:${CONTACT.email}`}>{CONTACT.email}</a><a href={`tel:${CONTACT.phone}`}>{CONTACT.phone}</a><span>{CONTACT.address}</span></div></div>
        </div>
